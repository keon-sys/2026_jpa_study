...
- JPA 상속 매핑은 프레임워크 차원에서 강제하고 있음 (SQL을 개발자가 쓰는 것과 JPA에 맡기는 것과 차이점?)
...
#### JPA와 객체 그래프 탐색
- JPA는 연관된 객체를 사용하는 시점에 적절한 SELECT SQL을 실행한다. -> 이를 *지연로딩* 이라고 한다. 
...
- 하이버네이트를 구현 프레임워크로 사용하는 JPA 2.1 버전에 대한 설명임
    - 현재는 3.x 까지 버전이 나옴, 여전히 하이버네이트 구현임
